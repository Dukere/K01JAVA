...
<<<<<<< HEAD
<<<<<<< HEAD
		String형 : 참조형변수로써 기본자료형이 아님.
			문자열을 저장할수 있는 데이터타입으로 실제로는 class이다.
			자바에서는 문자열을 표현할때 " 으로 감싸면되고, 연결할때
			+ 기호를 사용한다. 
		 */
		int number;
		number = 99;
		...
		단, 산술연산의 결과를 출력하고 싶다면 아래와같이 소괄호를 이용해서
		우선순위에 대한 변화를 주면된다.
		 */
...
=======
		 ...
		 문자열을 저장할 수 있는 데이터타입으로 실제로는 class이다.
		 자바에서는 문자열을 표현할 때 " 으로 감싸면 되고, 연결할 때 + 기호를 사용한다.
		 */
